...
(function(root, factory) {
		...
		if (typeof define === "function" && define.amd) { // AMD mode
			define(["underscore", "jquery", "json2"], factory);
		...
		Transport.factory = function(url, options) {
			var
				scheme = url.split(':')[0],
				_class = Transport.registry.uriScheme[scheme];

			if (typeof _class === 'undefined')
				throw new BaseError('Unknown transport scheme: ' + scheme);
			// console.log('Transport.factory: ',_class.name,' found for ', scheme);
			return new _class(url, options);
		}
		...
		_.extend(Transport.prototype, {
			...
			'_dispatchRequest': function(verb, urn, rqOptions) {
				try {
					var 
						...
						mappedMethod = assertDefined(this._methodsMap[verb], 
							'Verb \"'+verb+'\" is not supported by transport '+this.__tname),

						o = _.extend({}, this.options(), rqOptions, {'data': data}),

						qryArgs = popAttr(o, 'qryArgs'),

						uri = this._resolveUri(urn, qryArgs, o);

					// validate data type:
					assertType(data, this._accepts, 
						'Invalid type of rqOptions.data: '
						+ (typeof data)
						+ '\nTypes allowed: '+ this._accepts.join(","));

					return this._request(mappedMethod, uri, o).then(function (rspData) {
						return this._decodeData(rspData)
					}.bind(this))
				} catch (e) {
			...
			'accepts': function (value) {
				if (typeof value === 'undefined') return this._accepts;
				this._accepts = value;
			...
			'_resolveUri': function (urn, qryArgs, rqOptions) {
				var 
					_location = this._url.replace(/[\/]+$/g, ''),
					_urn = urn.replace(/^[\/]+/g, '');
				if (qryArgs) {
					_urn = [_urn, '?', serializeUriVariables(qryArgs)].join('')
				}
				console.log('_path-->', _urn);
				return this._escapePath([_location, '/',_urn].join(''));
			},
			...
			'options': function (value) {
				if (typeof value === 'undefined') return this._options;
				extendOptions(this._options, assertType(value, _.isObject), 'Transport.options error');
				return this;
			},
			...
			'methodsMap': function (valsHash) {
					if (typeof valsHash === 'undefined') return _.copy(this._methodsMap);
					...
					var unknownMethods = _.difference(_.values(valsHash), _.keys(this));
					if(unknownMethods.length>0)
						throw new Error('methodsMap error: Unknown methods(s): '+unknownMethods.join(','));
					_.extend(this._methodsMap, valsHash);
					return this; // for chaining
				},
			...
			'encodeTo': function(value) {
					if (typeof value === 'undefined') return this._encodeTo;
					this._encodeTo = assertType(value, ['json', 'xml', 'text', 'form', 'js-object'], 'Transport.encodeTo error');
					return this; // for chaining
				},
			...
			'_decodeData': function (data) {
				...
				switch (this._decodeFrom) {
					...
					default:
						DecodeError('No conversion defined for: '+this._decodeFrom+' from: '+dType);
				}
			},
			...
			'create': function(urn, rqOptions) {
				return this._dispatchRequest('create', urn, rqOptions)
			},

			'update': function(urn, rqOptions) {
				return this._dispatchRequest('update', urn, rqOptions)
			},

			'read': function(urn, rqOptions) {
				return this._dispatchRequest('read', urn, rqOptions)
			},

			'delete': function(urn, rqOptions) {
				return this._dispatchRequest('delete', urn, rqOptions)
			},
