    ...
    function deactivate(item, close, settings, dfd, setter) {
        if (item && item.deactivate) {
            ...
            try {
                ...
                system.log('ERROR #1: ' + error.message, error);
                dfd.resolve(false);
                return;
            }
    ...
    function activate(newItem, activeItem, callback, activationData) {
        ...
        if(newItem && newItem.activate) {
            ...
            try {
                ...
                system.log('ERROR #2: ' + error.message, error);
                callback(false);
                return;
            }
        }
        ...
        if(result && result.then) {
            result.then(function() {
                ...
                system.log('ERROR #3: ' + reason.message, reason);
                callback(false);
            });
        } else {
            activeItem(newItem);
            ...
            function continueCanDeactivate() {
                if (item && item.canDeactivate && options.canDeactivate) {
                    ...
                    try {
                        ...
                        system.log('ERROR #4: ' + error.message, error);
                        dfd.resolve(false);
                        return;
                    }
                    ...
                    if (resultOrPromise.then) {
                        resultOrPromise.then(function (result) {
                            ...
                            system.log('ERROR #5: ' +  reason.message, reason);
                            dfd.resolve(false);
                        });
                    } else {
        ...
        return system.defer(function (dfd) {
            ...
            if (newItem && newItem.canActivate) {
                ...
                try {
                    ...
                    system.log('ERROR #6: ' + error.message, error);
                    dfd.resolve(false);
                    return;
                }
                ...
                if (resultOrPromise.then) {
                    resultOrPromise.then(function(result) {
                        ...
                        system.log('ERROR #7: ' + reason.message, reason);
                        dfd.resolve(false);
                    });
                } else {
                    settings.lifecycleData = resultOrPromise;
